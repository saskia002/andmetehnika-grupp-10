...
-- TOBEDELETED BEFORE SUBMIT at ERIK:  I updated to new structure, please review

SELECT 
    c.Industry,
    SUM(f.Sales) AS total_revenue
FROM DimCompany c
JOIN FactFinancials f ON f.CompanyKey = c.CompanyKey AND toDate(concat(f.Year, '-12-31')) BETWEEN c.ValidFrom AND c.ValidTo
JOIN Forbes_2000.FactForbesRank r ON r.CompanyKey = c.CompanyKey AND r.Year = f.Year
WHERE r.ForbesRank <= 2000
GROUP BY c.Industry
...
-- TOBEDELETED BEFORE SUBMIT at ERIK:  siin ma motlesin et arvutad aasta lopu hind miinus algus ja siis votad keskmise üle industry and siis milline neid keskmistest on suurim
